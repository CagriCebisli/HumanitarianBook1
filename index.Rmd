...
There are several different methods for sampling. I personally like machine learning style of sampling. 
...
diamonds_index <- createDataPartition(diamonds$cut, p = .25, 
                                  list = FALSE,
                                  times = 1)
...
Now that we draw sample. It is .25 of the total population. But, we have to compare population metrics with sample metrics. As stressed in scenario paragraph, we are trying to understand socio-economic variables from sample, and then we will assume these findings applies to whole population. 
...
Compare <- rbind(diamonds_sample,diamonds)
...
Let's double check with numeric variables and compare them by type. Depth and Table variables taken into consider. Check how both linear models aligns with each other perfectly. Here, we can assume that our sample does really well!
